class ProductCataloguesController < ApplicationController
  ...
  def order_on_whats
    ...
    redirect_to "https://wa.me/55#{@product_catalogue.catalogue.whatsapp_number}?text=Olá, tenho interesse em #{quantity} unidades de #{product.name}, #{product_variant.name} -> link #{catalogue_product_catalogue_url(@product_catalogue.catalogue, @product_catalogue)}"
  end
